 ...
 public:
  ...
  void onPublish(const ros::TimerEvent&){
    ...
    AV1World.setOrigin(tf::Vector3(cos(time), sin(time), 0.0));
        tf::Quaternion q1;
        q1.setRPY(0, 0, time); // Orientación para mantener el eje Y tangente a la trayectoria
    ...
    // *** FILL IN ***
    tf_broadcaster.sendTransform(tf::StampedTransform(AV1World, ros::Time::now(), "world", "av1"));
    tf_broadcaster.sendTransform(tf::StampedTransform(AV2World, ros::Time::now(), "world", "av2"));
